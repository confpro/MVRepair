...
from transformers import AdamW, get_linear_schedule_with_warmup, AutoTokenizer,AutoModelForCausalLM, LlamaTokenizer, LlamaForCausalLM
        ...
        for cur_epoch in range(int(num_train_epochs)):
            ...
            if do_eval:
                ...
                if do_eval_bleu:
                    ...
                    # 将ref_list转换为DataFrame并存储为CSV
                    ref_df = pd.DataFrame(ref_list, columns=['reference'])
                    ref_df.to_csv('ref_list.csv', index=False)

                    # 将hyp_list转换为DataFrame并存储为CSV
                    hyp_df = pd.DataFrame(hyp_list, columns=['hypothesis'])
                    hyp_df.to_csv('hyp_list.csv', index=False)
